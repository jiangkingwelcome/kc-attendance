...
import com.kc.system.service.ISysPostService;
...
public class SysUserController extends BaseController
{
    ...
    public AjaxResult add(@Validated @RequestBody SysUser user)
    {
        ...
        else if (!StringUtils.equals(user.getEmployeeId(),"-1") && employeeService.checkIdAndName(user.getEmployeeId(),user.getEmployeeId()))
        {
            return error("新增用户'" + user.getUserName() + "'失败，员工id和员工姓名不匹配！");
        }
        if (StringUtils.equals(user.getEmployeeId(),"-1")){
            for (Long roleId : user.getRoleIds()) {
                if (roleId != null  && roleId.equals(102L)){
                    return error("新增用户'" + user.getUserName() + "'失败，员工id为-1的用户不能拥有员工角色！");
                }
            }
        }
